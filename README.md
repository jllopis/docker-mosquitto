...
Default for **REPOSITORY** is **jllopis/mosquitto** (should change this) and for **TAG** is **mosquitto version (1.4.2 now)**.

Actually the command executed by make is

    docker build --no-cache -t jllopis/mosquitto:1.4.2 .
...
(or whatever user u have configured...)
    ...
    $ docker run -ti --rm jllopis/mosquitto:1.4.2 np -p secretpass
