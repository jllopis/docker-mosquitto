...
You can specify your repository and tag by
    ...
    $ sudo make REPOSITORY=my_own_repo/mqtt TAG=v1.4.12

Default for **REPOSITORY** is **jllopis/mosquitto** (should change this) and for **TAG** is **mosquitto version (1.4.12 now)**.

Actually the command executed by make is

    docker build --no-cache -t jllopis/mosquitto:v1.4.12 .
...
(or whatever user u have configured...)
    ...
    $ docker run -ti --rm jllopis/mosquitto:v1.4.12 np -p secretpass
